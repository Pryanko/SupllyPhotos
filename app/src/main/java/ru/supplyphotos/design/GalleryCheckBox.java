...
import ru.supplyphotos.tools.utils.CompatUtils;

/**
 * @author Libgo on 02.04.2018.
 */
public class GalleryCheckBox extends View implements Checkable{

    private static final String KEY_INSTANCE_STATE = "InstanceState";

    private static final int COLOR_TICK = Color.parseColor("#ffffbe75");
    private static final int COLOR_UNCHECKED = Color.parseColor("#00FFFFFF");
    private static final int COLOR_CHECKED = Color.WHITE;
    private static final int COLOR_FLOOR_UNCHECKED = Color.parseColor("#00FFFFFF");
    ...
    public GalleryCheckBox(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public GalleryCheckBox(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public GalleryCheckBox(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.GalleryCheckBox);
        int tickColor = ta.getColor(R.styleable.GalleryCheckBox_color_tick, COLOR_TICK);
        mAnimDuration = ta.getInt(R.styleable.GalleryCheckBox_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(R.styleable.GalleryCheckBox_color_unchecked_stroke, COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(R.styleable.GalleryCheckBox_color_checked, COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(R.styleable.GalleryCheckBox_color_unchecked, COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.GalleryCheckBox_stroke_width, CompatUtils.dp2px(getContext(), 0));
        ta.recycle();
    ...
    public void setChecked(boolean checked) {
        ...
        invalidate();
        if (mListener != null) {
            mListener.onCheckedChanged(GalleryCheckBox.this, mChecked);
        }
    }
    ...
    public interface OnCheckedChangeListener {
        void onCheckedChanged(GalleryCheckBox checkBox, boolean isChecked);
    }
